...
async function loadCustomTemplate() {
  try {
    ...
    // Return basic template structure instead of referencing reportTemplates
    return {
      name: 'Custom Report',
      includeMetrics: true,
      includeEngagement: true,
...
async function fetchWithPlaywright(keyword, maxPosts, includeComments, startTime) {
  ...
  try {
    ...
    const searchUrl = `https://x.com/search?q=${encodeURIComponent(keyword)}&src=typed_query&f=${values.sortBy === 'top' ? 'top' : 'live'}`;
    ...
    try {
      ...
      console.log(`  ✅ Tweet elements detected on page`);
    } catch (e) {
      console.log(`  ⚠️ No tweets found initially, waiting longer...`);
      await page.waitForTimeout(5000);
    }
...
async function extractPosts(page, maxPosts) {
  ...
  while (posts.length < maxPosts && scrollAttempts < maxScrollAttempts) {
    // Add delay before each extraction attempt
    if (scrollAttempts > 0) {
      console.log(`  ⏳ Waiting before next extraction attempt...`);
      await page.waitForTimeout(3000);
    }
    
    const newPosts = await page.evaluate(() => {
      const tweets = document.querySelectorAll('[data-testid="tweet"]');
      console.log(`Found ${tweets.length} tweet elements on page`);
    ...
    // Scroll to load more
    console.log(`  📜 Scrolling to load more posts (attempt ${scrollAttempts + 1}/${maxScrollAttempts})...`);
    ...
    const filterMsg = values.englishOnly 
      ...
      : '';
    console.log(`  📊 Found ${newPosts.length} new posts, collected ${posts.length}/${maxPosts} total${englishFiltered > 0 ? filterMsg : ''}...`);
  }
